...
public interface OrderRepository extends JpaRepository<UserOrder, String> {
    ...
    @Query(value="select user_order.userid,product.name ,product.price,user_order.num ,user_order.status,user_order.id from ( user_order left join product on (user_order.productid=product.id)) where user_order.status=?1",
            nativeQuery = true)
