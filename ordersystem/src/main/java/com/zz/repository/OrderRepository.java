...
public interface OrderRepository extends JpaRepository<UserOrder, String> {
    ...
    @Query(value="select user_order.id,user_order.userid,product.name ,product.price,user_order.num ,user_order.createtime,user_order.status from ( user_order left join product on (user_order.productid=product.id)) where user_order.status=?1",
            nativeQuery = true)
    Page<Object[]> confirmRefundByStatus(String status,Pageable pageable);

    @Query(value="select userid,name,price,num,status,id FROM (select user_order.id,user_order.userid,product.name ,product.price ,user_order.num ,user_order.sum,user_order.status,user_order.createtime,user_order.completetime from ( user_order left join product on (user_order.productid=product.id)) where  user_order.userid=?1 or product.name=?1 or user_order.id=?1) as a where status=?2 ",
            nativeQuery = true)
    Page<Object[]> adminsRefundSelectOrder(String parm,String status,Pageable pageable);

    @Query(value ="select id,userid,name,price,num,createtime,status FROM (select id,userid,name,price,num,sum,createtime,completetime,status FROM (select user_order.id,user_order.userid,product.name ,product.price ,user_order.num ,user_order.sum,user_order.status,user_order.createtime,user_order.completetime from ( user_order left join product on (user_order.productid=product.id)) where  user_order.userid=?1 or product.name=?1 or user_order.id=?1) AS a where status like CONCAT('%',?2,'%') )as b where str_to_date(createtime, '%Y-%m-%d') BETWEEN str_to_date(?3, '%Y-%m-%d') AND str_to_date(?4, '%Y-%m-%d') OR str_to_date(completetime, '%Y-%m-%d') BETWEEN str_to_date(?3, '%Y-%m-%d') AND str_to_date(?4, '%Y-%m-%d')\n", nativeQuery = true)
    Page<Object[]> adminsRefundSelectOrder(String parm,String status,String starttime,String finishtime,Pageable pageable);
